...
export async function POST(request: Request) {
  // Top-level try/catch — qualquer exception vira JSON 500 em vez de body vazio
  try {
    ...
    let parsed;
    try {
      parsed = parseSourceDoc(body.source);
    } catch (err) {
      console.error("[generate] parseSourceDoc falhou:", err);
      return NextResponse.json(
        { ok: false, error: `Falha ao parsear doc: ${(err as Error).message}` },
        { status: 400 },
      );
    }
    ...
    // Confere se as env vars críticas existem antes de tentar processar
    if (!process.env.CLAUDE_KEY) {
      return NextResponse.json(
        { ok: false, error: "CLAUDE_KEY não configurada no servidor (env var)" },
        { status: 500 },
      );
    }
    if (!process.env.PEXELS_API_KEY) {
      return NextResponse.json(
        { ok: false, error: "PEXELS_API_KEY não configurada no servidor (env var)" },
        { status: 500 },
      );
    }
    ...
    try {
      ...
      console.error("[generate] saveDraft falhou:", err);
      return NextResponse.json(
        { ok: false, error: `Falha ao salvar draft (storage?): ${(err as Error).message}` },
        { status: 500 },
      );
    }
    ...
    console.log(`[generate] OK — draftId=${draft.id} ads=${skeletonAds.length}`);
    return NextResponse.json({ ok: true, draftId: draft.id });
  } catch (err) {
    console.error("[generate] CRASH inesperado:", err);
    return NextResponse.json(
      { ok: false, error: `Erro inesperado: ${(err as Error).message}` },
      { status: 500 },
    );
  }
}
